...
- **数据管理**：统一设置数据文件位置和名称，备份目录可独立配置，并支持数据迁移、自动备份、手动备份和恢复。
- **本地存储**：财务数据保存在用户指定的 Excel 文件中，不上传到云端。
- **更新检查**：启动后静默读取 GitHub 上的 sha256 校验文件；无网络或访问失败时不影响使用。
...
- “数据”负责文件：可修改数据目录、Excel 文件名和独立备份目录，迁移数据、设置备份数量、手动备份及恢复。备份目录留空时仍保存在数据文件旁边。
- “参数”负责软件：可修改界面字号，开启或关闭启动更新检查，也可手动检查更新。
